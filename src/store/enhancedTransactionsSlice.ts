  ...
  async ({ userId, accountId }: { userId: string; accountId?: string }) => {
    try {
      ...
      return transactions.map((t: any) => ({
        ...
        transfer_detection_confidence: t.transfer_detection_confidence,
  ...
  async (transactionData: Omit<Transaction, 'id'>) => {
    try {
      ...
      return {
        ...
        transfer_detection_confidence: undefined,
  ...
  async ({ id, updates }: { id: string; updates: Partial<Transaction> }) => {
    try {
      ...
      return {
        ...
        // New transfer linking fields
        linked_transaction_id: updates.linked_transaction_id,
        transfer_pair_id: updates.transfer_pair_id,
        is_internal_transfer: updates.is_internal_transfer || false,
        transfer_detection_confidence: updates.transfer_detection_confidence,
        transfer_notes: updates.transfer_notes,
